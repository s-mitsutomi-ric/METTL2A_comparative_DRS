...
plot_intensity_distribution_at_each_position <- function(
  ...
  df_filtered <- df |> 
    filter(ref_pos == pos)  |> 
    group_by(sample_num) |> 
    filter(intensity > quantile(intensity, probs = .025), na.rm = TRUE)  |> 
    filter(intensity < quantile(intensity, probs = .975), na.rm = TRUE) |> 
    ungroup()
  ...
  figdir_dist <- paste0(figdir, 'Intensity_density_95quantile/', id, '_', tr_name, '/')
  ...
  plot_distribution <- df_filtered |> 
    ggplot(aes(x = intensity, 
               fill = paste(type, si))) +
    geom_density(alpha = 1/2, lwd = 0.1) +
    labs(title = plot_title) +
    scale_fill_manual(values = c('grey', "#75C6C3", '#F266AB')) +
  ...
  figdir_ecdf <- paste0(figdir, 'Ecdf_95quantile/', id, '_', tr_name, '/')
  ...
  plot_ecdf <- df_filtered |> 
    ggplot(aes(x = intensity, colour = paste(type, si))) +
    stat_ecdf(alpha = 1/2) +
    labs(x = 'intensity', title = plot_title) +
    scale_color_manual(values = c('grey', "#75C6C3", '#F266AB')) +
    theme_classic(base_size = 7) +
    theme(legend.position = 'bottom', legend.title = element_blank())  
